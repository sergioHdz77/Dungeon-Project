...
var run_savings_label: Label = null
...
func build_ui() -> void:
	...
	run_savings_label = Label.new()
	vbox.add_child(run_savings_label)
...
func update_hud(
	...
	if health_bar == null or stamina_bar == null:
		return
	...
	var run_savings: int = _get_int_from_player(player, "get_run_savings", 0)
	...
	run_savings_label.text = "Ahorro meta: %s" % run_savings

	weapon_label.text = "Arma: %s" % weapon_name
	damage_label.text = "Dano: %s" % int(damage)
	block_label.text = "Bloqueo: activo" if is_blocking else "Bloqueo: no"

	health_bar.max_value = maxf(max_health, 1.0)
	health_bar.value = clampf(health, 0.0, health_bar.max_value)
	health_value_label.text = "Vida: %s / %s" % [int(health), int(max_health)]

	stamina_bar.max_value = maxf(max_stamina, 1.0)
	stamina_bar.value = clampf(stamina, 0.0, stamina_bar.max_value)
	stamina_value_label.text = "Stamina: %s / %s" % [int(stamina), int(max_stamina)]
